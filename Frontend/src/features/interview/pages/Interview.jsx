...
const RoadMapDay = ({ day }) => (
    <div className='roadmap-day'>
        <div className='roadmap-day-header'>\
            <span className='roadmap-day_badge'>Day {day.day}</span>
            <h3 className='roadmap-day_focus'>{day.focus}</h3>
        </div>
        <ul className='roadmap-day_tasks'>
            {day.tasks.map((task, i) => (
                <li key={i}>
                    <span className='roadmap-day_bullet' />
                    {task}
                </li>
            ))}
        </ul>
            ...
            <div className='interview-layout'>
                ...
                <nav className='interview-nav'>
                    <div className='nav-content'>
                        <p className='interview-nav_label'>Sections</p>
                        {NAV_ITEMS.map(item => (
                            <button
                                key={item.id}
                                className={`interview-nav_item ${activeNav === item.id ? 'interview-nav_item--active' : ''}`}
                                onClick={() => setActiveNav(item.id)}
                            >
                                <span className='interview-nav_icon'>{item.icon}</span>
                                {item.label}
                            </button>
                        ))}
                    </div>
                ...
                <aside className='interview-sidebar'>
                    <div className='match-score'>
                        <p className='match-score_label'> Match Score</p>
